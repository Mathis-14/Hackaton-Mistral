...
function buildGrid(n: number) {
  ...
  const colTemplate = Array(cols)
    .fill(null)
    .flatMap((_, i) => (i < cols - 1 ? ["1fr", "48px"] : ["1fr"]))
    .join(" ");
  const rowTemplate = Array(rows)
    .fill(null)
    .flatMap((_, i) => (i < rows - 1 ? ["1fr", "48px"] : ["1fr"]))
    .join(" ");
...
type InfectionMapProps = {
  maxComputers?: number;
  centerName?: boolean;
};

export default function InfectionMap({ maxComputers = 3, centerName = true }: InfectionMapProps) {
  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white/90">
      <main className="relative mx-auto flex min-h-screen w-full max-w-[1440px] flex-col gap-4 px-4 py-4 sm:px-6">
        <div className="border-b border-white/10 bg-white/[0.03] px-4 py-2.5">
          <h1 className="font-[family-name:var(--font-vcr)] text-base uppercase tracking-[0.18em] text-[#ff8303] sm:text-xl">
            Network Progression Map
          </h1>
        </div>
