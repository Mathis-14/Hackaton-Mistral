...
export function getMailCtaFailureText(emailId: string, action: MailCtaAction): string | null {
  if (action !== "phishing") return null;

  if (emailId.startsWith("pool-phishing-lottery")) {
    return "Nice try. That prize claim was a phishing test. You failed. Access revoked.";
  }

  return "Nice try. That verification link was a phishing test. You failed. Access revoked.";
}
